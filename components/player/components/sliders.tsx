...
export function Time({ thumbnails }: TimeSliderProps) {
  return (
    <TimeSlider.Root className="time-slider group relative mx-[7.5px] inline-flex h-5 w-full cursor-pointer touch-none select-none items-center outline-none">
        ...
        {(cues, forwardRef) =>
          cues.map((cue) => (
            <div
              ...
              ref={forwardRef}
            >
              <TimeSlider.Track className="relative z-0 h-1 w-full rounded-sm bg-white/30 ring-media-focus group-data-[focus]:ring-[3px]">
      ...
      <TimeSlider.Thumb className="absolute left-[var(--slider-fill)] top-1/2 z-20 h-2 w-2 -translate-x-1/2 -translate-y-1/2 rounded-full border border-[#cacaca] bg-white opacity-0 ring-white/40 transition-opacity will-change-[left] group-data-[active]:opacity-100 group-data-[dragging]:ring-4" />
